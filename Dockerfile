...
# Ensure Cargo-installed binaries (e.g., cbindgen) are in PATH
ENV PATH="/root/.cargo/bin:${PATH}"

# cbindgen is required by Suricata's build to generate Rust bindings
...
RUN ./autogen.sh && \
    ...
    if [ -d /usr/local/etc/suricata ]; then cp -a /usr/local/etc/suricata/. /etc/suricata/; fi && \
    ldconfig
