...
use crate::core::{self, ALPROTO_UNKNOWN, IPPROTO_UDP};
use crate::direction::Direction;
use crate::flow::Flow;
use std::ffi::CString;
use std::os::raw::{c_char, c_void};
use suricata_sys::sys::{
    AppLayerParserState, AppProto, AppProtoNewProtoFromString, SCAppLayerParserConfParserEnabled,
...
/// ALPROTO id assigned during registration.
pub static mut ALPROTO_STUN: AppProto = ALPROTO_UNKNOWN;
...
pub unsafe extern "C" fn SCRegisterStunParser() {
    ...
    ALPROTO_STUN = AppProtoNewProtoFromString(PARSER_NAME.as_ptr() as *const c_char);
    let _ = AppLayerRegisterProtocolDetection(&parser, 1);
    if SCAppLayerParserConfParserEnabled(ip_proto_str.as_ptr(), parser.name) != 0 {
        let _ = AppLayerRegisterParser(&parser, ALPROTO_STUN);
    }
    SCAppLayerParserRegisterLogger(IPPROTO_UDP, ALPROTO_STUN);
